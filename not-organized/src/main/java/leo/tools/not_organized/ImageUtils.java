...
public class ImageUtils {
    private static final Logger log = Logger.getLogger(ImageUtils.class.toString());
    ...
    public static String scale(String src, int w) throws IOException {
        int inserTo = src.lastIndexOf(".");
        String dest = src.substring(0, inserTo) + String.format("_%s", w) + src.substring(inserTo, src.length());
        scale(src, dest, w);
        return dest;
    }


    public static void scale(String src, String dest, int w) throws IOException {
        ...
        BufferedImage srcBuffered = reader.read(0);

        int srcWidth = srcBuffered.getWidth();
        int srcHeight = srcBuffered.getHeight();

        int h = w * srcHeight / srcWidth;
    ...
    public static void pressImage(String watermarkImg, String srcImageFile) {
        pressImage(watermarkImg, srcImageFile, srcImageFile, 5, -1, -1, 0.2f, 1);
    }

    public static void xpressImage(String watermarkImg, String srcImageFile, String destImageFile) {
        pressImage(watermarkImg, srcImageFile, destImageFile, 5, -1, -1, 0.2f, 1);
    }

    public static void pressImage(String watermarkImg, String srcImageFile, String destImageFile, int position,
                                  float alpha) {
     ...
     * @param position      水印打印的位置： 1->左上角，2->右上角，1->居中，1->左下角，1->右下角
        ...
        try {
            ...
            switch (position) {
                ...
                case 2:
                    ...
                    break;
                case 3:
                    xPostion = (srcWidth - newWidth) / 2;
                    yPostion = (srcHeight - newHeight) / 2;
                    break;
                case 4:
                    xPostion = xOffset;
                    yPostion = srcHeight - newHeight - yOffset;
                    break;
                case 5:
                    xPostion = (int) (srcWidth * (1 - radio) - xOffset);
                    yPostion = srcHeight - newHeight - yOffset;
            ...
            log.severe("ImageUtils pressImage error");
        }
    }
